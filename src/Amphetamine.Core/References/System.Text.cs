...
namespace System.Text
{
	...
	{
		public static string FirstCharToUpper(this string input) => string.IsNullOrEmpty(input) ? input : input.First().ToString().ToUpper() + input.Substring(1);

		public static string[] SplitWords(this string inputString) => inputString.Split('"').Select((Func<string, int, string[]>)((element, index) => index % 2 != 0 ? new string[1] {element} : element.Split(new char[1] {' '}, StringSplitOptions.RemoveEmptyEntries))).SelectMany((Func<string[], IEnumerable<string>>)(element => element)).ToArray();
